-- Incremental migration: WhatsApp renewal reminder dry-run support.
...
begin
  ...
  values (
    ...
    concat(
      ...
      'Status: ', new.status,
      case when new.overdue_days > 0 then concat(' • Overdue ', new.overdue_days, ' day', case when new.overdue_days = 1 then '' else 's' end) else '' end,
      case when new.payment_link_url <> '' then concat(' • Payment link: ', new.payment_link_url) else ' • Awaiting parent plan choice' end
    ),
    new.created_by
  );
