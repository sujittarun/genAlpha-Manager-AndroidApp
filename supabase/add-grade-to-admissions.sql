-- 1. Add grade column to admissions table
alter table public.admissions add column if not exists grade text not null default '';

-- 2. Update submit_admission_form RPC to include p_grade
-- We follow the exact parameter order and logic from schema.sql
create or replace function public.submit_admission_form(
  ...
  p_grade text, -- Added grade parameter
...
begin
  ...
  values (
    ...
    p_address,
    p_school_college,
    coalesce(p_grade, ''), -- Added p_grade
    p_parent_aadhaar_no,
    p_time_slot,
    p_join_date,
    p_fees_paid,
...
-- 3. Update sync_student_from_admission trigger function to include grade
...
begin
  ...
  values (
    ...
    new.school_college,
    coalesce(new.grade, ''), -- Now syncing grade correctly
    new.address,
    '{}',
    false,
    null,
...
$$;
-- Note: Existing sync function doesn't check review_status, it probably runs on review_status change.
-- Let's check the full sync_student_from_admission in schema.sql.
