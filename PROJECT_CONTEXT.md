...
- Reminder rows are cycle-scoped. Anything stored on a `reminder_events` / `payment_link_requests` row (`manual_followup_required`, `manual_followup_reason`, delivery status, selected plan, amount, cycle) describes ONE billing cycle and expires with it. It may only drive today's UI when the row belongs to the player's current cycle — see `isFollowUpForCurrentCycle` in `web-app-repo/reminder-cycle-rules.js` and `data/ReminderCycleRules.kt`, which are pinned to the shared `reminder-cycle-fixtures.json`.
- Live player state is never cycle-gated: `overdue >= 15 days`, `whatsapp_contact_status` (`wrong_number` / `opted_out`), `whatsapp_reminders_paused`, fees pending and renewal pending are recomputed from the `students` row every time. Gating those would hide real manual follow-ups.
- Every overdue count starts from `currentFeeCycleDate` (`currentDueDate` in Kotlin): the rejoin-aware paid-through date for renewals, and `max(join_date, rejoined_at)` when the joining fee is still owed. Do not read `students.join_date` directly for overdue math — a rejoined player would be permanently overdue.
